function tableSort (tableid) {
	...
	var tbHeadButton = tbHead.find('button');
	var tbBody = tableObject.children('tbody');// 获取table对象下的tbody
	var tbBodyTr = tbBody.find('tr');// 获取tbody下的tr
	var sortIndex = -1; // 初始化索引
	tbHeadButton.each(function() {// 遍历thead的tr下的th
		var thisIndex = $(this)[0].id.split("-")[0];// 获取th所在的列号
		
		$(this).click(function() { // 鼠标点击事件
			var dataType = $(this).attr("type"); // 获取当前点击列的 type
			checkColumnValue(thisIndex, dataType); // 对当前点击的列进行排序
														// （当前索引，排序类型）
		});
	});
			...
			if (index == sortIndex) {// sortIndex =-1
				...
				for (var j = i + 1; j < len; j++) {
					...
					value2 = trsValue[j].split(".separator")[1];// 当前值的下一个
						if (type == "number") {
							// js 三元运算 如果 values1 等于 '' （空） 那么，该值就为0，否则
							// 改值为当前值
							...
							if (parseFloat(value1) > parseFloat(value2)) {// 如果当前值
																			// 大于
																			// 下一个值
							...
							// 用本地特定的顺序来比较两个字符串。
							// 说明比较结果的数字。
							// 如果 stringObject 小于 target，则 localeCompare()
							// 返回小于 0 的数。
		...
		function ip2int(ip) {
			...
			ip = ip.split(".");
			// Number() 函数把对象的值转换为数字。
			num = Number(ip[0]) * 256 * 256 * 256 + Number(ip[1]) * 256
					* 256 + Number(ip[2]) * 256 + Number(ip[3]);
			return num;
		}
}